...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    Pred Anterior volume: {inference['anterior']}
    Pred Posterior volume: {inference['posterior']}
    """
    draw.multiline_text((10, 90), f"{display_des}",
                        (255, 255, 255), font=main_font)
    ...
    for i in range(len(slice_nums)): 
        nd_img = np.flip(overlay_norm[:, :, slice_nums[i]]).T.astype(np.uint8)
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((350, 400))
        pimg.paste(pil_i, box=(10+325*i, 400))
        pil_i.save(os.path.join("..", "out", header.PatientID+"_"+dt+'_slice'+str(slice_nums[i])+'.png'), format='png')

    
#     nd_o = np.flip((orig_vol[0, :, :]/np.max(orig_vol[0, :, :]))*oxff).T.astype(np.uint8)
#     pil_o = Image.fromarray(nd_o, mode = "L").convert("RGBA").resize((400,400))
#     pimg.paste(pil_o, box=(50, 500))
    
#     nd_p = np.flip((pred_vol[0, :, :]/np.max(pred_vol[0, :, :]))*0xff).T.astype(np.uint8)
#     pil_p = Image.fromarray(nd_p, mode="L").convert("RGBA").resize((400,400))
#     pimg.paste(pil_p, box=(550, 500))
...
def get_series_for_inference(path):
    ...
    if len({f.SeriesInstanceUID for f in series_for_inference}) != 1: 
        print("Error: No clue what series to run inference on")
        return []
...
if __name__ == "__main__":
    ...
    dte = datetime.date.today().strftime("%Y%m%d")
    tme = datetime.date.now().strftime("%H%M%S")
    ...
    os_command(f"storescu localhost 4242 -v -aec HIPPOAI +r +sd '/home/workspace/out/report.dcm'")
